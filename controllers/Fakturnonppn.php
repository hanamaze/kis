...
class Fakturnonppn extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('Fakturnonppn_model');
    ...
    public function index()
    {
        ...
        $config['total_rows'] = $this->Fakturnonppn_model->total_rows($q);
        $faktur = $this->Fakturnonppn_model->get_limit_data($q);
        ...
        $data = array(
            ...
            'start' => $start,
            'konten' => 'fakturnonppn/faktur_list',
        );
    ...
    public function read() 
    {
        ...
        $row = $this->Fakturnonppn_model->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'transaksi' => $row->transaksi,
            ...
            $this->load->view('fakturnonppn/faktur_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('fakturnonppn'));
        }
    }
	public function optional() 
    {
		$this->load->view('fakturnonppn/optional');
	}
public function cetak($id) 
    {
		...
		'transaksi' => $row->transaksi,
            ...
            $this->load->view('fakturnonppn/faktur_cetak', $data);
	...
	public function cetak2($id) 
    {
		...
		'transaksi' => $row->transaksi,
            ...
            $this->load->view('fakturnonppn/faktur_cetak2', $data);
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('fakturnonppn/create_action'),
        ...
        $this->load->view('fakturnonppn/faktur_form', $data);
    }
    
    public function create_action() 
    {
        
		$this->load->view('fakturnonppn/terbilang');
        
		$tahun = SUBSTR($this->input->post('tanggal'),6,9);
		
		$k_f = "Inv-KIS";
		$k_k = "Kwt-KIS";
		$n = $this->db->query("SELECT * FROM fakturnonppn WHERE SUBSTRING(tanggal,7,10)='$tahun' ORDER BY no_faktur DESC")->row();
		if($n != NULL){
		...
		'transaksi' => $this->input->post('transaksi',TRUE),
            ...
            $this->Fakturnonppn_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('fakturnonppn/update_action'),
        ...
        'konten' => 'fakturnonppn/faktur_form',
	    );
            $this->load->view('index', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
    ...
    public function update_action() 
    {
		...
		'transaksi' => $this->input->post('transaksi',TRUE),
            ...
            $this->Fakturnonppn_model->update($this->input->post('id_faktur', TRUE), $data);
    ...
    public function persetujuan() 
    {
    	...
    	if($this->input->post('setujui')){
            ...
            $this->Fakturnonppn_model->update($id_faktur, $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Fakturnonppn_model->delete($id);
    ...
    public function word()
    {
        ...
        $data = array(
            'faktur_data' => $this->Fakturnonppn_model->get_all(),
            'start' => 0
        );
        
        $this->load->view('fakturnonppn/faktur_doc',$data);
    }
