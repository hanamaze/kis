<?php
 $path =  '<?php echo base_url(); ?>skl/header_surat.jpg';
...
<?php 
   ...
   if($tgl_mulai=="01"){
    ...
    $tanggal_selesai = $plt->tanggal_selesai[0].$plt->tanggal_selesai[1]." ".$tgl_2." ".$thn_selesai;
		...
		if($pl->id_peserta != NULL){
		if($surat != NULL){
			$no_surat = md5_decrypt($surat->no_surat);
		}else{
			$no_surat = "Nomor Tidak Ditemukan";
		}
if($plt->id_kategori_pelatihan==1){
  $keterangan = " Keputusan Penunjukan (SKP), sertifikat dan kartu lisensi ";
}elseif($plt->id_kategori_pelatihan==2){
  $keterangan = " Sertifikat";
}elseif($plt->id_kategori_pelatihan==3){
$keterangan = " Sertifikat dan kartu lisensi ";
}elseif($plt->id_kategori_pelatihan==4){
  $keterangan = " Keputusan Penunjukan (SKP), sertifikat dan kartu lisensi ";
}elseif($plt->id_kategori_pelatihan==5){
  $keterangan = " Keputusan Penunjukan (SKP), sertifikat dan kartu lisensi ";
}elseif($plt->id_kategori_pelatihan==6){
  $keterangan = " Sertifikat dan kartu lisensi";
}elseif($plt->id_kategori_pelatihan==7){  
  $keterangan = " Sertifikat";
}elseif($plt->id_kategori_pelatihan==8){
  $keterangan = " Keputusan Penunjukan (SKP), sertifikat dan kartu lisensi ";
}elseif($plt->id_kategori_pelatihan==9){
  $keterangan = " Keputusan Penunjukan (SKP), sertifikat dan kartu lisensi ";
}else{
  $keterangan = " Keputusan Penunjukan (SKP), sertifikat dan kartu lisensi ";
}
...
<img src="skl/header_surat.jpg" style="width:803px;">
...
<img src="skl/footer_surat.jpg" style="width:803px;">
